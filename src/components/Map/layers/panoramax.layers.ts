...
export const PANORAMAX_TILE_URL = `${process.env.NEXT_PUBLIC_PANORAMAX_API_URL}/api/map/{z}/{x}/{y}.mvt`;
...
export const PANORAMAX_SEQUENCE_HIGHLIGHT_LAYER_ID =
  "panoramax-sequences-highlight";
...
export const findNearestPictureForSequence = (
  ...
  for (const f of features) {
    ...
    const coords = (f.geometry as any)?.coordinates as
      | [number, number]
      | undefined;
  ...
  for (const line of lines) {
    for (let i = 0; i < line.length - 1; i++) {
      ...
      let t =
        len2 === 0
          ? 0
          : ((clickPx.x - a.x) * dx + (clickPx.y - a.y) * dy) / len2;
      if (t < 0) t = 0;
      else if (t > 1) t = 1;
      const px = a.x + t * dx;
      const py = a.y + t * dy;
