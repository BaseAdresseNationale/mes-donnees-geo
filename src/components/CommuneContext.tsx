...
export function CommuneProvider({
  ...
  return <CommuneContext.Provider value={value}>{children}</CommuneContext.Provider>;
}

export function useCommune(): CommuneContextValue {
  const ctx = useContext(CommuneContext);
  if (!ctx) throw new Error("useCommune doit être utilisé à l'intérieur d'un CommuneProvider.");
  return ctx;
}
