...
export interface AssembledRuralPath {
...
function bdTopoSegment(portion: Portion): AssembledSegment {
  return {
    path: portion.path,
    source: RuralPathSource.BD_TOPO,
    sourceRef: portion.candidate.cleabs,
    surface: portion.candidate.suggestedSurface,
    largeurMoyenne: portion.candidate.suggestedLargeurMoyenne,
    domanialite: portion.candidate.suggestedDomanialite,
  };
}
...
export function assembleRuralPathsFromCadastre(
  ...
  const assembled: AssembledRuralPath[] = [];
